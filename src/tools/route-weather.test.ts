...
import { computeRouteWeather } from "./route-weather.js";
...
describe("computeRouteWeather — sampleRoute behavior", () => {
  it("always includes Start and Ziel points", async () => {
    ...
    const result = await computeRouteWeather(
      "key", "A", "B", "DRIVE",
      new Date("2026-02-19T10:00:00Z"), 30,
    );
  ...
  it("correctly interpolates lat/lng within a step", async () => {
    ...
    await computeRouteWeather(
      "key", "A", "B", "DRIVE",
      new Date("2026-02-19T10:00:00Z"), 30,
    );

    // The 30-min weather fetch should be for the interpolated midpoint
    const weatherCalls = mockFetch.mock.calls.filter((c) =>
      typeof c[0] === "string" && c[0].includes("open-meteo"),
    );
    ...
    expect(parseFloat(url.searchParams.get("latitude")!)).toBeCloseTo(48.5, 1);
    expect(parseFloat(url.searchParams.get("longitude")!)).toBeCloseTo(11.5, 1);
  });
});
...
describe("computeRouteWeather — formatOffsetLabel", () => {
  it("formats 30 min as '30 Min.'", async () => {
    mockFetch
      .mockResolvedValueOnce(routeResponse([
        { startLat: 48, startLng: 11, endLat: 49, endLng: 12, dur: 3600 },
      ], 3600))
  ...
  it("formats 60 min as '1 Std.'", async () => {
    ...
    mockFetch
      .mockResolvedValueOnce(routeResponse([
        { startLat: 48, startLng: 11, endLat: 49, endLng: 12, dur: 7200 },
      ], 7200))
      .mockResolvedValueOnce(weatherResponse(5, 0, 0, 10))
      .mockResolvedValueOnce(weatherResponse(4, 1, 5, 12))
      .mockResolvedValueOnce(weatherResponse(3, 2, 10, 14))
...
describe("computeRouteWeather — WMO codes", () => {
  it("maps WMO code 0 to clear sky", async () => {
    mockFetch
      .mockResolvedValueOnce(routeResponse([
        { startLat: 48, startLng: 11, endLat: 48.1, endLng: 11.1, dur: 600 },
      ], 600))
